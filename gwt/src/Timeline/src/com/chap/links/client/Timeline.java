...
implements Selectable {
	 ...
	 * Create an instance via the method create, for example
	 *	 Timeline.Options options = Timeline.Options.create();
	 */
	public static class Options extends AbstractDrawOptions {
		public enum STYLE {BOX, DOT};
		 ...
		 * @param width	 Width as a string, for example "100%" or "500px"
		 */
		 ...
		 * WHen height is set to "auto", the height of the Timeline is automatically
		 * adjusted to fit the contents. See also setMinHeight to set a minimum
		 * height.
		 * @param height	 Height as a string, for example "100%" or "500px", or
		 *								 "auto"
		 */
...
//			}
		 ...
		 * @param style	Choose from BOX (default), or DOT
		 */
		public final void setStyle(STYLE style) {
			switch (style) {
			case BOX:	nativeSetStyle("box"); break;
			case DOT:	nativeSetStyle("dot"); break;
			default:	 nativeSetStyle("box");
			}
		}
		...
		/**
		 * If set to true, a navigation menu is shown, containing buttons to move
		 * and zoom the timeline. Default value is false.
		 * @param show
		 */
		 ...
		 * n an event is selected, the select event is fired.
		 * @param selectable
		 */
	 ...
	 *									end (type DATETIME),
	 *									content (type STRING)
	 * @param options A name/value map containing settings for the timeline.
	 *								See the class Timeline.Options for all available options
	 */
	 ...
	 * The current status of the timeline will be preserved (in contrast with
	 * the method draw)
	 * @param data
	 */
	 ...
	 * return item
	 */
	private native JavaScriptObject nativeGetItem(JavaScriptObject jso, int index) /*-{
