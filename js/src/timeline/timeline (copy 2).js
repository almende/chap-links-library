...
links.Timeline.prototype.recalcItems = function () {
    ...
    if (groups.length == 0) {
        ...
        if (options.autoHeight || options.cluster) {
            ...
            if (size.actualHeight != actualHeight && !options.axisOnTop) {
...
/**
 * Uncluster the clustered items
 */
links.Timeline.prototype.unclusterItems = function () {
...
/**
 * Cluster the events which are located above the maximum allowed height
 */
links.Timeline.prototype.clusterItems = function () {
    var maxHeight = 300; // TODO: make maxHeight customizable

    var clusters = [];

    // filter the items which have a top larger than maxHeight
    var sortedItems = this.stackOrder(this.visibleItems);
    var finalItems = this.stackCalculateFinal(sortedItems);
    var axisOnTop = this.options.axisOnTop;
    var contentHeight = this.size.contentHeight;
    var overflowingItems = [];
    finalItems.forEach(function (finalItem) {
        // TODO: reckon with a maximum height here
        if (axisOnTop) {
            if (finalItem.bottom > contentHeight) {
                overflowingItems.push(finalItem);
                finalItem.overflow = true;
            }
        }
        else {
            if (finalItem.top < 30) {  // TODO: what margin to take?
                overflowingItems.push(finalItem);
                finalItem.overflow = true;
            }
        }
    ...
    // sort the overflowing items by top
    overflowingItems.sort(function (a, b) {
        return a.top - b.top;
    });

    //console.log('overflowingItems', {'overflowingItems': overflowingItems}, overflowingItems.length); // TODO: cleanup

    // calculate the distance between two items
    var distance = function (a, b) {
        var aCenter = (a.left + a.right) / 2;
        var bCenter = (b.left + b.right) / 2;
        return Math.abs(aCenter - bCenter);
    };

    // merge the items with an other, non-overflowing item
    var timeline = this;
    overflowingItems.forEach(function (finalItem) {
        // find the closest other item
        var closestItem = undefined;
        var closestDist = undefined;
        delete finalItem.overflow;
        finalItems.forEach(function (other) {
            if (other != finalItem && !other.overflow) {
                var dist = distance(finalItem, other);
        ...
        // merge with the closest other item
        if (closestItem) {
            var closest = closestItem.item,
            ...
            console.log('no closest item found for item=', finalItem);
        }
    });

    // calculate count and average start for the cluster
    clusters.forEach(function (cluster) {
        var start = undefined;
        var count = 0;
        cluster.items.forEach(function (item) {
