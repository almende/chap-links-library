...
links.Timeline.prototype.redrawItems = function() {
    var i, iMax, item, domItem, left, right;
    ...
    // create or update the items DOM
    for (i = 0, iMax = items.length; i < iMax; i++) {
        item = items[i];
        // create a new dom when needed
        switch (item.type) {
            case 'range':
                if (!item.dom) {
                    // create a new range
                    item.dom = this.createEventRange(item);
                    frame.appendChild(item.dom);
                    item.changed = true;
                }
                if (item.changed) {
            ...
            case 'box':
                if (!item.dom) {
                    // create a new box
                    item.dom = this.createEventBox(item);
                    frame.appendChild(item.dom);
                    frame.insertBefore(item.dom.line, frame.firstChild);
                    // Note: line must be added in front of the items,
                    //       such that it stays below all items
                    frame.appendChild(item.dom.dot);
                    item.changed = true;
                }
                if (item.changed) {
            ...
            case 'dot':
                if (!item.dom) {
                    // create a new box
                    item.dom = this.createEventDot(item);
                    frame.appendChild(item.dom);
                    item.changed = true;
                }
                if (item.changed) {
    ...
    //console.log('redrawItems after creation visibleitems=', items)
    ...
    for (i = 0, iMax = items.length; i < iMax; i++) {
        ...
        switch (item.type) {
            case 'range':
                ...
                if (item.visible || size.dataChanged) { // TODO: remove size.dataChanged here?
...
links.Timeline.prototype.recalcSize = function() {
    ...
    if (size.dataChanged || true) { // TODO: always calculate the size of an item?
        //if (true) {
        groupsWidth = 0;
...
links.Timeline.prototype.onMouseMove = function (event) {
    ...
    if (params.customTime) {
        ...
        if (this.groups.length == 0) {
            ...
            this.filterItems();
            this.stackItems(options.animate);
            if (!options.animate) {
                this.redrawFrame();
            }
...
links.Timeline.prototype.deleteItem = function(index) {
    ...
    this.clusterItems();
    this.filterItems();
    this.stackItems(this.options.animate);
...
links.Timeline.prototype.stackItems = function(animate) {
    ...
    if (animate) {
        ...
        animation.sortedItems = sortedItems;
        animation.finalItems = finalItems;

        var timeline = this;
        var step = function () {
            var arrived = timeline.stackMoveOneStep(animation.sortedItems,
...
links.Timeline.prototype.filterItemTypes = function () {
    ...
    // filter on ranges and point items
    ...
    for (var i = 0, iMax = items.length; i < iMax; i++) {
        ...
        switch (item.type) {
            case 'range':
                ...
                item.visible = (right > -contentWidth)
                    && (left < 2 * contentWidth)
                    && (!item.cluster);
                break;
            ...
            case 'box':
                ...
                item.visible = ((left + item.width/2 > -contentWidth)
            ...
            case 'dot':
                ...
                item.visible = (left + item.width > -contentWidth)
                    && (left < 2 * contentWidth)
                    && (!item.cluster);
                break;
...
links.Timeline.prototype.clusterItems = function () {
    ...
    while (i < max) {
        ...
        if (neighbors > maxItems) {
            ...
            cluster.content = 'Cluster<br>(' + count + ' items)';
